...
def get_data():
    ...
    try:
        ...
        LEFT JOIN kho k WITH (NOLOCK) ON s.[ID Cuộn Bó] = k.[ID Cuộn Bó] -- Đặt tên tắt cho bảng kho là 'k'
        WHERE 
        ...
        UNION ALL
        SELECT [ID Cuộn Bó], [Vị trí], [Khối lượng], Nhóm, [SO Mapping] FROM kho WITH (NOLOCK)"""
...
def simulate_warehouse_state():
    df = pd.read_sql("""select s.[ID Cuộn Bó], s.[Vị trí]
                ...
                UNION ALL
                SELECT [ID Cuộn Bó], [Vị trí] FROM kho WITH (NOLOCK)""", engine)
    df = df.fillna('')
    df['pos_len'] = df['Vị trí'].astype(str).str.len()
    df['priority'] = df['Vị trí'].apply(calculate_priority)
        # df = df.sort_values(by='pos_len', ascending=True) 
...
0 as [SO Mapping]
from sanluong s WITH (NOLOCK)
LEFT JOIN kho WITH (NOLOCK) k ON s.[ID Cuộn Bó] = k.[ID Cuộn Bó] -- Đặt tên tắt cho bảng kho là 'k'
WHERE 
    s.[ID Cuộn Bó] IS NOT NULL AND s.[ID Cuộn Bó] <> ''
    AND (s.[Đã nhập kho] = 'No' OR s.[Đã nhập kho] IS NULL)
    AND k.[ID Cuộn Bó] IS NULL
